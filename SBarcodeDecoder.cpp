...
Result ReadBarcode(const QImage& img, const DecodeHints& hints = {})
{
    ...
    auto exec = [&](const QImage& img) {
        //qWarning() << "img fmt" << img.format() << hex << (int)ImgFmtFromQImg(img);
...
void SBarcodeDecoder::setCaptured(const QString &captured)
{
    ...
    _captured = captured;
    Q_EMIT capturedChanged(_captured);
}
...
void SBarcodeDecoder::setIsDecoding(bool isDecoding)
{
    ...
    Q_EMIT isDecodingChanged(_isDecoding);
}
...
{
    //qWarning() << "frame" << videoFrame.pixelFormat() << videoFrame.planeCount() << videoFrame.handleType();
    if (videoFrame.handleType() == QAbstractVideoBuffer::NoHandle) {
        ...
        return image.copy(captureRect).convertToFormat(QImage::Format_Grayscale16);
    }
